...
class LiveDemucs(private val context: Context) : AutoCloseable {
    private val env = OrtEnvironment.getEnvironment(); private val sessionOptions = OrtTuning.createOptions(); private var session: OrtSession? = null
    ...
    @Synchronized private fun ensureSession(): OrtSession { session?.let { return it }; val model = assetFile("demucs_4s.onnx"); return env.createSession(model.absolutePath, sessionOptions).also { session = it } }
