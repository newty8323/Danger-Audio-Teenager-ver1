...
`no_speech` 기반 자동 문장 삭제는 45개 뉴스 구간에서 한 번도 작동하지 않아 채택하지 않았다. 정확도 관련 처리는 이전 상태로 되돌렸다. 2스레드 1시간 시험은 평균 CPU를 낮췄지만 p95 RTF 1.109, p95 처리시간 4,437.8 ms, 최대 대기 깊이 3으로 실시간 기준을 통과하지 못했다. 따라서 모델, 임계값, 4초 분석 창, decoder 상한과 반복 종료 정책은 그대로 두고 ONNX intra-op만 3개로 설정해 4스레드의 실시간성과 2스레드의 CPU 절감 사이를 비교한다. 동일 콘텐츠와 시작 온도를 맞추고 p95 RTF, 미처리 창, CPU, 온도와 thermal 상태를 측정하며, p95 RTF가 1 이상이거나 대기열이 누적되면 3스레드 설정도 채택하지 않는다.
