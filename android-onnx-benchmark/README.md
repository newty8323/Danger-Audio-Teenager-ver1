...
`no_speech` 기반 자동 문장 삭제는 45개 뉴스 구간에서 한 번도 작동하지 않아 채택하지 않았다. 정확도 관련 처리는 이전 상태로 되돌리고, CPU·발열에 직접 영향을 주는 ONNX intra-op 스레드만 4개에서 2개로 줄였다. 모델, 임계값, 4초 분석 창, decoder 상한과 반복 종료 정책은 동일하다. 동일 콘텐츠 구간을 사용해 4스레드 결과와 p95 RTF, 미처리 창, CPU, 온도와 thermal 상태를 비교하며, 실시간 조건을 지키지 못하면 2스레드 설정은 채택하지 않는다.
